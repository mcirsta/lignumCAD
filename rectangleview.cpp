...
public:
    ...
    RectangleViewCreate ( RectangleView* rectangle_view )
    {
        ...
        root.setAttribute( lC::STR::NAME, rectangle_view->dbURL().toString() );
    ...
    bool mergeRename ( RenameCommand* rename )
    {
        ...
        if ( memento_list.length() > 0 ) {
            ...
            root.setAttribute( lC::STR::NAME, rename->newDBURL().toString() );
...
public:
    ...
    void write ( QDomDocument* document ) const
    {
        ...
        restyle_element.setAttribute( lC::STR::NAME, db_url_.toString() );
...
void RectangleCreateInput::startDisplay ( QMenu* context_menu )
{
    ...
    QAction* cancel_action =
            rectangle_view_->parent()->lCMW()->getUi()->cancelRectangleAction;
                                          ...
                                          const SelectedNames& selected )
{
    ...
    QAction* cancel_action = page_view->lCMW()->getUi()->cancelRectangleAction;
                                          ...
                                          const SelectedNames& selected )
{
    ...
    QListIterator< std::shared_ptr<ConstrainedLine> >  edgeIt ( edges_);
    while ( edgeIt.hasNext() ){
        ConstrainedLine* edge = edgeIt.next().get();
        if ( x_reference != 0 && edge->isVertical() ) {
            ...
            if ( !x_reference->dependsOn( edge ) ) {
                ...
                old_xml.setAttribute( lC::STR::URL, edge->dbURL().toString() );
                ...
                new_xml.setAttribute( lC::STR::URL, edge->dbURL().toString() );
      ...
      style_( 0 )
{
    ...
    list_view_item_->setData( trC( lC::STR::RECTANGLE_FMT ).
                              ...
                              lC::DETAIL );
    x0_list_view_->setData( rectangle_->x0()->detail(), lC::DETAIL );
    y0_list_view_->setData( rectangle_->y0()->detail(), lC::DETAIL );
    x1_list_view_->setData( rectangle_->x1()->detail(), lC::DETAIL );
    y1_list_view_->setData( rectangle_->y1()->detail(),  lC::DETAIL );
                    ...
                    if ( !dimension_view_node.isNull() ) {
                        ...
                        if ( !dimension_view_element.isNull() &&
                            ...
                            dimensionview_objects_.insert( x0_dimensionview_->selectionName(),
                                                           std::shared_ptr<DimensionView>(x0_dimensionview_) );
                    ...
                    if ( !dimension_view_node.isNull() ) {
                        ...
                        if ( !dimension_view_element.isNull() &&
                            ...
                            dimensionview_objects_.insert( y0_dimensionview_->selectionName(),
                                                           std::shared_ptr<DimensionView>(y0_dimensionview_) );
                    ...
                    if ( !dimension_view_node.isNull() ) {
                        ...
                        if ( !dimension_view_element.isNull() &&
                            ...
                            dimensionview_objects_.insert( x1_dimensionview_->
                                                           selectionName(),
                                                           std::shared_ptr<DimensionView>(x1_dimensionview_) );
                    ...
                    if ( !dimension_view_node.isNull() ) {
                        ...
                        if ( !dimension_view_element.isNull() &&
                            ...
                            dimensionview_objects_.insert( y1_dimensionview_->
                                                           selectionName(),
                                                           std::shared_ptr<DimensionView>(y1_dimensionview_) );
...
InputObject* RectangleView::modifyInput ( void )
{
    ...
    while ( idmv.hasNext() ) {
        if ( idmv.peekNext().value()->isActivated() ) {
            return idmv.peekNext().value()->modifyInput();
        }
        idmv.next();
    }
...
void RectangleView::init ( void )
{
...
#else
    list_view_item_->setData( lC::formatName( rectangle_->name() )
                              + QString( " <%1>" ).arg( rectangle_->id() ),
                              lC::NAME );
#endif
    list_view_item_->setData( trC( lC::STR::RECTANGLE ), lC::TYPE );
    ...
    x0_list_view_->setData( lC::formatName( rectangle_->x0()->name() ), lC::NAME );
    x0_list_view_->setData( trC( lC::STR::CONSTRAINED_LINE ), lC::TYPE );
    x0_list_view_->setData( trC( lC::STR::CONSTRAINT_UNDEFINED ), lC::DETAIL );
    ...
    y0_list_view_->setData( lC::formatName( rectangle_->y0()->name() ), lC::NAME );
    y0_list_view_->setData( trC( lC::STR::CONSTRAINED_LINE ), lC::TYPE );
    y0_list_view_->setData( trC( lC::STR::CONSTRAINT_UNDEFINED ), lC::DETAIL );
    ...
    x1_list_view_->setData( lC::formatName( rectangle_->x1()->name() ), lC::NAME );
    x1_list_view_->setData( trC( lC::STR::CONSTRAINED_LINE ), lC::TYPE );
    x1_list_view_->setData( trC( lC::STR::CONSTRAINT_UNDEFINED ), lC::DETAIL );
    ...
    y1_list_view_->setData( lC::formatName( rectangle_->y1()->name() ), lC::NAME );
    y1_list_view_->setData( trC( lC::STR::CONSTRAINED_LINE ), lC::TYPE );
    y1_list_view_->setData( trC( lC::STR::CONSTRAINT_UNDEFINED ), lC::DETAIL );
...
//    connect( list_view_item_, SIGNAL( nameChanged( const QString& ) ),this,
//             SLOT( listNameChanged( const QString& ) ) );

//    connect( x0_list_view_, SIGNAL( nameChanged( const QString& ) ),this,
//             SLOT( x0SetName( const QString& ) ) );
//    connect( y0_list_view_, SIGNAL( nameChanged( const QString& ) ),this,
//             SLOT( y0SetName( const QString& ) ) );
//    connect( x1_list_view_, SIGNAL( nameChanged( const QString& ) ),this,
//             SLOT( x1SetName( const QString& ) ) );
//    connect( y1_list_view_, SIGNAL( nameChanged( const QString& ) ),this,
                                     ...
                                     const std::vector<GLuint>& items )
{
    if ( entity == FIGURE ) {
        ...
        if ( items.size() > 1 && rectangle_objects_.find(items[1]) !=  rectangle_objects_.end() )
...
{
    if ( dimensionview_objects_.find( selection_name ) != dimensionview_objects_.end() )
        return dimensionview_objects_[ selection_name ]->isActivated();
...
{
    QHashIterator< int,GraphicsView > igv( rectangle_objects_ );
...
/*!
   ...
   */
QList< std::shared_ptr<ConstrainedLine> > RectangleView::dragEdges ( GLuint selection_name ) const
{
    QList< std::shared_ptr<ConstrainedLine> > edges;

    if ( selection_name == lb_handle_.selectionName() ) {
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->left()) );
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->bottom()) );
    }
    else if ( selection_name == mb_handle_.selectionName() ) {
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->bottom()) );
    }
    else if ( selection_name == rb_handle_.selectionName() ) {
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->right()) );
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->bottom()) );
    }
    else if ( selection_name == mr_handle_.selectionName() ) {
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->right()) );
        ...
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->right()) );
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->top()) );
    }
    else if ( selection_name == mt_handle_.selectionName() ) {
        edges.append( std::shared_ptr<ConstrainedLine>(rectangle_->top()) );
    }
...
{
    rectangle_info_dialog_->getUi()->nameEdit->
    ...
    if ( width_constraint != 0 ) {
        rectangle_info_dialog_->getUi()->widthButtonGroup->setEnabled( true );
        rectangle_info_dialog_->getUi()->widthSpecifiedRadioButton->setChecked( true );
    }
    else {
        rectangle_info_dialog_->getUi()->widthButtonGroup->setEnabled( false );
    }

    rectangle_info_dialog_->getUi()->widthLengthSpinBox->
    ...
    rectangle_info_dialog_->getUi()->widthLengthSpinBox->setLength( width );

    // For now, this operation is not allowed...
    rectangle_info_dialog_->getUi()->widthImportedRadioButton->setEnabled( false );
    rectangle_info_dialog_->getUi()->widthConstraintChooser->setEnabled( false );
    ...
    if ( height_constraint != 0 ) {
        rectangle_info_dialog_->getUi()->heightButtonGroup->setEnabled( true );
        rectangle_info_dialog_->getUi()->heightSpecifiedRadioButton->setChecked( true );
    }
    else {
        rectangle_info_dialog_->getUi()->heightButtonGroup->setEnabled( false );
    }

    rectangle_info_dialog_->getUi()->heightLengthSpinBox->
            setLengthLimits( UnitsBasis::instance()->lengthUnit(),
                             UnitsBasis::instance()->format(),
                             UnitsBasis::instance()->precision(),
                             lC::MINIMUM_DIMESION, lC::MAXIMUM_DIMENSION, height );
    rectangle_info_dialog_->getUi()->heightLengthSpinBox->setLength( height );

    // For now, this operation is not allowed...
    rectangle_info_dialog_->getUi()->heightImportedRadioButton->setEnabled( false );
    rectangle_info_dialog_->getUi()->heightConstraintChooser->setEnabled( false );
    ...
    if ( x_constraint != 0 ) {
        rectangle_info_dialog_->getUi()->xOriginButtonGroup->setEnabled( true );
        rectangle_info_dialog_->getUi()->xOriginSpecifiedRadioButton->setChecked( true );
    }
    else {
        rectangle_info_dialog_->getUi()->xOriginButtonGroup->setEnabled( false );
    }

    rectangle_info_dialog_->getUi()->xOriginReferenceLineEdit->
            setText( rectangle_->x0()->reference()->path() );
    rectangle_info_dialog_->getUi()->xOriginOffsetLengthSpinBox->
            setLengthLimits( UnitsBasis::instance()->lengthUnit(),
                             UnitsBasis::instance()->format(),
                             UnitsBasis::instance()->precision(),
                             lC::MINIMUM_DIMESION, lC::MAXIMUM_DIMENSION, x );
    rectangle_info_dialog_->getUi()->xOriginOffsetLengthSpinBox->setLength( x );

    // For now, this operation is not allowed...
    rectangle_info_dialog_->getUi()->xOriginImportedRadioButton->setEnabled( false );
    rectangle_info_dialog_->getUi()->xOriginConstraintChooser->setEnabled( false );
    ...
    if ( y_constraint != 0 ) {
        rectangle_info_dialog_->getUi()->yOriginButtonGroup->setEnabled( true );
        rectangle_info_dialog_->getUi()->yOriginSpecifiedRadioButton->setChecked( true );
    }
    else {
        rectangle_info_dialog_->getUi()->yOriginButtonGroup->setEnabled( false );
    }

    rectangle_info_dialog_->getUi()->yOriginReferenceLineEdit->
            setText( rectangle_->y0()->reference()->path() );
    rectangle_info_dialog_->getUi()->yOriginOffsetLengthSpinBox->
            setLengthLimits( UnitsBasis::instance()->lengthUnit(),
                             UnitsBasis::instance()->format(),
                             UnitsBasis::instance()->precision(),
                             lC::MINIMUM_DIMESION, lC::MAXIMUM_DIMENSION, y );
    rectangle_info_dialog_->getUi()->yOriginOffsetLengthSpinBox->setLength( y );

    // For now, this operation is not allowed...
    rectangle_info_dialog_->getUi()->yOriginImportedRadioButton->setEnabled( false );
    rectangle_info_dialog_->getUi()->yOriginConstraintChooser->setEnabled( false );

    switch ( width_annotation_side_  ) {
    case lC::ABOVE:
        rectangle_info_dialog_->getUi()->aboveButton->setChecked( true );
        break;
    case lC::BELOW:
        rectangle_info_dialog_->getUi()->belowButton->setChecked( true );
        break;
    }

    switch ( height_annotation_side_  ) {
    case lC::ABOVE:
        rectangle_info_dialog_->getUi()->rightButton->setChecked( true );
        break;
    case lC::BELOW:
        rectangle_info_dialog_->getUi()->leftButton->setChecked( true );
        break;
    }

    if ( style_ != 0 )
        rectangle_info_dialog_->getUi()->rectangleStyleSlider->setValue( style_->style() );
    else
        rectangle_info_dialog_->getUi()->rectangleStyleSlider->setValue( 0 );

    switch ( rectangle_info_dialog_->getUi()->rectangleStyleSlider->value() ) {
    case lC::Render::PARENT:
        rectangle_info_dialog_->getUi()->defaultStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->edgeStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->edgeStyleComboBox->setEnabled( false );
        rectangle_info_dialog_->getUi()->wireframeStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->wireframeStyleColorChooser->setEnabled( false );
        rectangle_info_dialog_->getUi()->solidStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->setEnabled( false );
        rectangle_info_dialog_->getUi()->textureStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->textureStyleFileChooser->setEnabled( false );

        rectangle_info_dialog_->getUi()->edgeStyleComboBox->
                setDefaultValue( lC::Render::SOLID );
        rectangle_info_dialog_->getUi()->edgeStyleComboBox->
                setValue( lC::Edge::SOLID );
        rectangle_info_dialog_->getUi()->wireframeStyleColorChooser->
                setDefaultColor( OpenGLGlobals::instance()->geometryColor() );
        rectangle_info_dialog_->getUi()->wireframeStyleColorChooser->
                setColor( OpenGLGlobals::instance()->geometryColor() );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->
                setDefaultColor( OpenGLGlobals::instance()->geometryColor() );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->
                setColor( OpenGLGlobals::instance()->geometryColor() );
        rectangle_info_dialog_->getUi()->textureStyleFileChooser->
    ...
    case lC::Render::STIPPLE:
        rectangle_info_dialog_->getUi()->defaultStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->edgeStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->edgeStyleComboBox->setEnabled( true );
        rectangle_info_dialog_->getUi()->wireframeStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->wireframeStyleColorChooser->setEnabled( false );
        rectangle_info_dialog_->getUi()->solidStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->setEnabled( false );
    ...
    case lC::Render::WIREFRAME:
        ...
        rectangle_info_dialog_->getUi()->edgeStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->edgeStyleComboBox->setEnabled( true );
        rectangle_info_dialog_->getUi()->wireframeStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->wireframeStyleColorChooser->setEnabled( true );
        rectangle_info_dialog_->getUi()->solidStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->setEnabled( false );
        rectangle_info_dialog_->getUi()->textureStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->textureStyleFileChooser->setEnabled( false );
    ...
    case lC::Render::SOLID:
        ...
        rectangle_info_dialog_->getUi()->wireframeStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->wireframeStyleColorChooser->setEnabled( true );
        rectangle_info_dialog_->getUi()->solidStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->setEnabled( true );
        rectangle_info_dialog_->getUi()->textureStyleLabel->setEnabled( false );
        rectangle_info_dialog_->getUi()->textureStyleFileChooser->setEnabled( false );
    ...
    case lC::Render::TEXTURED:
        ...
        rectangle_info_dialog_->getUi()->solidStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->solidStyleColorChooser->setEnabled( true );
        rectangle_info_dialog_->getUi()->textureStyleLabel->setEnabled( true );
        rectangle_info_dialog_->getUi()->textureStyleFileChooser->setEnabled( true );
...
redo:
    ...
    if ( rectangle_info_dialog_->getUi()->nameEdit->isModified() ) {
        ...
        setName( rectangle_info_dialog_->getUi()->nameEdit->text() );
        modified = true;
    }

    MoveLinesCommand* move_command = 0;

    if ( rectangle_info_dialog_->getUi()->widthLengthSpinBox->edited() ) {
        ...
        if ( width_constraint->offset() >= 0 )
            width_constraint->
                    setOffset( rectangle_info_dialog_->getUi()->widthLengthSpinBox->length() );
        else
            width_constraint->
                    setOffset( -rectangle_info_dialog_->getUi()->widthLengthSpinBox->length() );
    ...
    if ( rectangle_info_dialog_->getUi()->heightLengthSpinBox->edited() ) {
        ...
        if ( height_constraint->offset() >= 0 )
            height_constraint->
                    setOffset( rectangle_info_dialog_->getUi()->heightLengthSpinBox->length() );
        else
            height_constraint->
                    setOffset( -rectangle_info_dialog_->getUi()->heightLengthSpinBox->length() );
    ...
    if ( rectangle_info_dialog_->getUi()->xOriginOffsetLengthSpinBox->edited() ) {
        ...
        if ( rectangle_->x0()->offset() >= 0 )
            rectangle_->x0()->
                    setOffset( rectangle_info_dialog_->getUi()->xOriginOffsetLengthSpinBox->length() );
        else
            rectangle_->x0()->
                    setOffset( -rectangle_info_dialog_->getUi()->xOriginOffsetLengthSpinBox->length() );
    ...
    if ( rectangle_info_dialog_->getUi()->yOriginOffsetLengthSpinBox->edited() ) {
        ...
        if ( rectangle_->y0()->offset() >= 0 )
            rectangle_->y0()->
                    setOffset( rectangle_info_dialog_->getUi()->yOriginOffsetLengthSpinBox->length() );
        else
            rectangle_->y0()->
                    setOffset( -rectangle_info_dialog_->getUi()->yOriginOffsetLengthSpinBox->length() );
    ...
    if ( rectangle_info_dialog_->getUi()->aboveButton->isChecked() &&
    ...
    else if ( rectangle_info_dialog_->getUi()->belowButton->isChecked() &&
    ...
    if ( rectangle_info_dialog_->getUi()->rightButton->isChecked() &&
    ...
    else if ( rectangle_info_dialog_->getUi()->leftButton->isChecked() &&
    ...
    if ( rectangle_info_dialog_->getUi()->rectangleStyleSlider->value() == 0 ) {
        if ( style_ != 0 ) {
        ...
        style->setStyle( (lC::Render::Style)rectangle_info_dialog_->getUi()->
                         rectangleStyleSlider->value() );
        style->setEdge( (lC::Edge::Style)rectangle_info_dialog_->getUi()->
                        edgeStyleComboBox->value() );
        style->setWireframeColor( rectangle_info_dialog_->getUi()->
                                  wireframeStyleColorChooser->color() );
        style->setSolidColor( rectangle_info_dialog_->getUi()->
                              solidStyleColorChooser->color() );
        style->setTextureFile( rectangle_info_dialog_->getUi()->
...
const
{
    ...
    GLdouble s_coefficients[] = { x_resolution, 0., 0.,
                                 -x_resolution * rectangle_->lbVertex()[X] };
    GLdouble t_coefficients[] = { 0., y_resolution, 0.,
...
{
    QHash< int,GraphicsView >::iterator graphics_object = rectangle_objects_.find( selection_name );

    if ( graphics_object == rectangle_objects_.end() ) {
        if( dimensionview_objects_.find( selection_name ) != dimensionview_objects_.end())
            view()->setCursor( QCursor( dimensionview_objects_[ selection_name ]->cursorShape() ) );
    }
    else
    ...
    if ( rectangle_->x0()->needsDimensionView() ) {
        ...
        if ( x0_dimensionview_ != 0 ) { // remove it if we have one
            ...
            if ( xml_doc != 0 ) {
                ...
                xml_rep.setAttribute( lC::STR::URL, rectangle_->dbURL().toString( ));
