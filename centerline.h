 ...
 * centerline.h
 *
 * Header for the Centerline classes
...
namespace Space2D {
  ...
  class Centerline : public Figure {
    Q_OBJECT
  ...
  public:
    Centerline ( uint id, const QString& name, Page* parent );
    Centerline ( uint id, const QDomElement& xml_rep, Page* parent );
    Centerline ( uint id, const QString& name, const QDomElement& xml_rep,
		 Page* parent );
    ~Centerline ( void );
    ...
    /*!
     * Adjust the position of the centerline by the given amount
     * (mostly for the benefit of the Paste adjustment).
     * \param delta distance in inches to move the centerline.
     */
  ...
  public:
    ...
    /*!
     * Create an XML representation of this centerline suitable for use
     * as a clipboard selection. The centerline is simplified;
