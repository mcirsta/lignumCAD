...
void SketchView::draw ( void ) const
{
  ...
  if ( !view()->printing() ) {
    ...
    Point o = view()->unproject2D( QPoint(0,0) );
    Vector d = view()->unproject2D( QPoint(10,10) ) - o;

    int n_log2 = ilogb( d[X] ) + 1;
    ...
    int i_start = (int)rint( floor( view()->llCorner()[X] / step ) );
    int i_end = (int)rint( ceil( view()->urCorner()[X] / step ) );
    int j_start = (int)rint( floor( view()->llCorner()[Y] / step ) );
    int j_end = (int)rint( ceil( view()->urCorner()[Y] / step ) );
	...
	glLineStipple( 1, dashes[0][view()->stipplePhase().y() % 16] );
      ...
      glBegin( GL_LINES );
      glVertex2d( i_x * step, view()->llCorner()[Y] );
      glVertex2d( i_x * step, view()->urCorner()[Y] );
      glEnd();
    }
	...
	glLineStipple( 1, dashes[0][view()->stipplePhase().x() % 16] );
      ...
      glBegin( GL_LINES );
      glVertex2d( view()->llCorner()[X], j_y * step );
      glVertex2d( view()->urCorner()[X], j_y * step );
      glEnd();
    }
...
public:
  ...
  {
    return lCMW->insertSketchAction;
  }
};
