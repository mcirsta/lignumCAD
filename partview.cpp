...
public:
  ...
  PartViewCreate ( PartView* part_view )
  {
    ...
    root.setAttribute( lC::STR::NAME, part_view->dbURL().toString(true) );
...
public:
  ...
  void write ( QDomDocument* document ) const
  {
    ...
    material_element.setAttribute( lC::STR::NAME, db_url_.toString(true) );
...
void PartView::init ( void )
{
  ...
  if ( material_dialog_ == 0 )
    material_dialog_ = new MaterialDialog( lCMW(), "material dialog" );

  if ( new_part_wizard_ == 0 )
    new_part_wizard_ = new NewPartWizard( lCMW(), "new part wizard" );
...
void PartView::show ( void ) const
{
  ...
  lCMW()->toolHoleAction->disconnect();
...
void PartView::write ( QDomElement& xml_rep ) const
{
  ...
  view_element.setAttribute( lC::STR::PART, part_->dbURL().toString( true ) );
