...
GLint gl2psGetVertex(GL2PSvertex *v, GLfloat *p){
  GLint i;
#if 0
  printf( "vertices according to OpenGL: %f %f %f\n", p[0], p[1], p[2] );
#endif
  v->xyz[0] = p[0];
  v->xyz[1] = p[1];
  v->xyz[2] = 1000. * p[2];
  ...
  while(used > 0){
	...
	if(v == 2){
	  if(boundary){
	    ...
	    flag = 0;
	  /*	  printf( "  which is added to the list\n" );*/
    ...
    case GL_PASS_THROUGH_TOKEN :
      ...
      case GL2PS_BEGIN_LINE_STIPPLE : dash=1; break;
	...
	break;
      case GL2PS_SET_LINE_STIPPLE :
	current+=2; 
	used-=2; 
	if ( dash != 0 )
	  dash=(GLuint)current[1];
	break;
      }
      current += 2; 
      used -= 2; 
...
GLvoid gl2psPrintPostscriptPrimitive(GLvoid *a, GLvoid *b){
  ...
  case GL2PS_LINE :
    ...
    break;
  case GL2PS_TRIANGLE :
    /*    printf( "Hey, printme i'm a triangle\n" );*/
    if(gl2ps.shade){
...
GLvoid gl2psPrintEMFHeader(GLvoid){
  RECT size;
  PSTR description;

  int title_len = strlen( gl2ps.title );
  int producer_len = strlen( gl2ps.producer );

  GL2PSEMF* emf = (GL2PSEMF*)gl2ps.stream;
  HDC context;

  description = gl2psMalloc( ( title_len + producer_len ) * sizeof( char ) + 3 );
  memset( description, 0, title_len + producer_len + 3 );
  strcpy( description, gl2ps.title );
  strcpy( &description[title_len+1], gl2ps.producer );

  // In lignumCAD, this is not meaningful.
  //  glGetIntegerv(GL_VIEWPORT, viewport);

  size.left = 0;
  size.right = emf->width * 2540;
  size.top = 0;
  size.bottom = emf->height * 2540;

  context = CreateEnhMetaFileWithFILEA( 0, emf->stream, &size, description );

  SetWindowOrgEx( context, 0, size.bottom, 0 );
  SetWindowExtEx( context, size.right, -size.bottom, 0 );

  emf->pen_color = RGB( 0, 0, 0 );
  emf->pen_width = 1;
  SelectObject( context, CreatePen( PS_SOLID, emf->pen_width, emf->pen_color ) );
  emf->brush_color = RGB( 0, 0, 0 );
  SelectObject( context, CreateSolidBrush( emf->brush_color ) );

  emf->context = context;
}
/*
 * Recursively subdivide a smooth shaded line segment to the given tolerance
 */
GLvoid gl2psPrintEMFLineSubDiv( GLshort dash, GL2PSvertex* v0, GL2PSvertex* v1 )
{
  if ( fabs( v0->rgba[0] - v1->rgba[0] ) > EMF_RED_TOLERANCE ||
       fabs( v0->rgba[1] - v1->rgba[1] ) > EMF_GREEN_TOLERANCE ||
       fabs( v0->rgba[2] - v1->rgba[2] ) > EMF_BLUE_TOLERANCE ) {

    GL2PSvertex sv0;

    sv0.xyz[0] = ( v0->xyz[0] + v1->xyz[0] ) / 2;
    sv0.xyz[1] = ( v0->xyz[1] + v1->xyz[1] ) / 2;
    sv0.rgba[0] = ( v0->rgba[0] + v1->rgba[0] ) / 2;
    sv0.rgba[1] = ( v0->rgba[1] + v1->rgba[1] ) / 2;
    sv0.rgba[2] = ( v0->rgba[2] + v1->rgba[2] ) / 2;

    gl2psPrintEMFLineSubDiv( dash, v0, &sv0 );
    gl2psPrintEMFLineSubDiv( dash, &sv0, v1 );
  }
  else {
    HPEN pen;
    COLORREF rgb = RGB( (int)(255 * ( v0->rgba[0] + v1->rgba[0] )/2),
			(int)(255 * ( v0->rgba[1] + v1->rgba[1] )/2),
			(int)(255 * ( v0->rgba[2] + v1->rgba[2] )/2) );
    GL2PSEMF* emf = (GL2PSEMF*)gl2ps.stream;

    if ( dash ) /* SO/OO appears to ignore this setting */
      pen = CreatePen( PS_DASH, 1, rgb );
    else
      pen = CreatePen( PS_SOLID, 1, rgb );

    SelectObject( emf->context, pen);

    MoveToEx( emf->context,
	      emf->scale_x * v0->xyz[0],
	      emf->scale_y * v0->xyz[1],
	      0 );
    LineTo( emf->context,
	    emf->scale_x * v1->xyz[0],
	    emf->scale_y * v1->xyz[1] );
  }
}
/*
 * Recursively subdivide a smooth shaded triangle to the given tolerance
 */
GLvoid gl2psPrintEMFTriSubDiv( GL2PSvertex* v0, GL2PSvertex* v1, GL2PSvertex* v2 )
{
  if ( fabs( v0->rgba[0] - v1->rgba[0] ) > EMF_RED_TOLERANCE ||
       fabs( v1->rgba[0] - v2->rgba[0] ) > EMF_RED_TOLERANCE ||
       fabs( v2->rgba[0] - v0->rgba[0] ) > EMF_RED_TOLERANCE ||
       fabs( v0->rgba[1] - v1->rgba[1] ) > EMF_GREEN_TOLERANCE ||
       fabs( v1->rgba[1] - v2->rgba[1] ) > EMF_GREEN_TOLERANCE ||
       fabs( v2->rgba[1] - v0->rgba[1] ) > EMF_GREEN_TOLERANCE ||
       fabs( v0->rgba[2] - v1->rgba[2] ) > EMF_BLUE_TOLERANCE ||
       fabs( v1->rgba[2] - v2->rgba[2] ) > EMF_BLUE_TOLERANCE ||
       fabs( v2->rgba[2] - v0->rgba[2] ) > EMF_BLUE_TOLERANCE ) {

    GL2PSvertex sv0, sv1, sv2;
    ...
    sv1.xyz[0] = ( v1->xyz[0] + v2->xyz[0] ) / 2;
    sv1.xyz[1] = ( v1->xyz[1] + v2->xyz[1] ) / 2;
    sv1.rgba[0] = ( v1->rgba[0] + v2->rgba[0] ) / 2;
    sv1.rgba[1] = ( v1->rgba[1] + v2->rgba[1] ) / 2;
    sv1.rgba[2] = ( v1->rgba[2] + v2->rgba[2] ) / 2;

    sv2.xyz[0] = ( v2->xyz[0] + v0->xyz[0] ) / 2;
    sv2.xyz[1] = ( v2->xyz[1] + v0->xyz[1] ) / 2;
    sv2.rgba[0] = ( v2->rgba[0] + v0->rgba[0] ) / 2;
    sv2.rgba[1] = ( v2->rgba[1] + v0->rgba[1] ) / 2;
    sv2.rgba[2] = ( v2->rgba[2] + v0->rgba[2] ) / 2;

    gl2psPrintEMFTriSubDiv( v0, &sv0, &sv2 );
    gl2psPrintEMFTriSubDiv( v1, &sv1, &sv0 );
    gl2psPrintEMFTriSubDiv( v2, &sv2, &sv1 );
    gl2psPrintEMFTriSubDiv( &sv0, &sv1, &sv2 );
  }
  else {
    POINT triangle[3];
    HPEN pen;
    HBRUSH brush;
    COLORREF rgb = RGB( (int)(255 *( v0->rgba[0] + v1->rgba[0] + v2->rgba[0] )/ 3),
			(int)(255 *( v0->rgba[1] + v1->rgba[1] + v2->rgba[1] )/ 3),
			(int)(255 *( v0->rgba[2] + v1->rgba[2] + v2->rgba[2] )/ 3));
    GL2PSEMF* emf = (GL2PSEMF*)gl2ps.stream; 

    pen = CreatePen( PS_SOLID, 0, rgb );
    SelectObject( emf->context, pen );
    brush = CreateSolidBrush( rgb );
    SelectObject( emf->context, brush );

    triangle[0].x = emf->scale_x * v0->xyz[0];
    triangle[0].y = emf->scale_y * v0->xyz[1];
    triangle[1].x = emf->scale_x * v1->xyz[0];
    triangle[1].y = emf->scale_y * v1->xyz[1];
    triangle[2].x = emf->scale_x * v2->xyz[0];
    triangle[2].y = emf->scale_y * v2->xyz[1];

    Polygon( emf->context, triangle, 3 );
  }
}

GLvoid gl2psPrintEMFPrimitive(GLvoid *a, GLvoid *b){
  GL2PSprimitive *prim;
  COLORREF rgb;
  HBRUSH brush;
  HPEN pen;
  ...
  switch(prim->type){
  case GL2PS_TEXT :
    rgb = RGB( (int)(255 * prim->verts[0].rgba[0]),
	       (int)(255 * prim->verts[0].rgba[1]),
	       (int)(255 * prim->verts[0].rgba[2]) );

    font = CreateFontA( -prim->text->fontsize * emf->scale_y, 0, 0, 0, FW_DONTCARE,
			0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
			CLIP_DEFAULT_PRECIS,
			DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
			prim->text->fontname );

    SelectObject( emf->context, font );
    SetBkMode( emf->context, TRANSPARENT );
    SetTextColor( emf->context, rgb );
    TextOutA( emf->context,
	      emf->scale_x * prim->verts[0].xyz[0],
	      emf->scale_y * prim->verts[0].xyz[1],
	      prim->text->str, strlen( prim->text->str ) );
    break;
  case GL2PS_POINT :
    rgb = RGB( (int)(255 * prim->verts[0].rgba[0]),
	       (int)(255 * prim->verts[0].rgba[1]),
    ...
    if ( emf->pen_width != 0 ) {
      pen = CreatePen( PS_NULL, 0, rgb );
      SelectObject( emf->context, pen );
      emf->pen_width = 0;
    }
    if ( rgb != emf->brush_color ) {
      brush = CreateSolidBrush( rgb );
      SelectObject( emf->context, brush );
      emf->brush_color = rgb;
    }
    ...
    Ellipse( emf->context,
	     emf->scale_x * ( prim->verts[0].xyz[0] - .5*prim->width ),
	     emf->scale_y * ( prim->verts[0].xyz[1] - .5*prim->width ),
	     emf->scale_x * ( prim->verts[0].xyz[0] + .5*prim->width ),
	     emf->scale_y * ( prim->verts[0].xyz[1] + .5*prim->width ) );
    break;
  case GL2PS_LINE :
    if(gl2ps.shade){
      gl2psPrintEMFLineSubDiv( prim->dash, &prim->verts[0], &prim->verts[1] );
    }
    else{
      rgb = RGB( (int)(255 * prim->verts[0].rgba[0]),
		 (int)(255 * prim->verts[0].rgba[1]),
      ...
      if ( rgb != emf->pen_color || emf->pen_width != 1 ) {
	if( prim->dash ) /* SO/OO appears to ignore this setting */
	  pen = CreatePen( PS_DASH, 1, rgb );
	else
	  pen = CreatePen( PS_SOLID, 1, rgb );
	...
	emf->pen_width = 1;
      ...
      MoveToEx( emf->context,
		...
		0 );
      LineTo( emf->context,
	      emf->scale_x * prim->verts[1].xyz[0],
	      emf->scale_y * prim->verts[1].xyz[1] );
    }
    break;
  case GL2PS_TRIANGLE :
    if(gl2ps.shade){
      gl2psPrintEMFTriSubDiv( &prim->verts[0], &prim->verts[1], &prim->verts[2] );
    }
    else{
      POINT triangle[3];
	...
	emf->brush_color = rgb;
      ...
      triangle[0].x = emf->scale_x * prim->verts[0].xyz[0];
      triangle[0].y = emf->scale_y * prim->verts[0].xyz[1];
      triangle[1].x = emf->scale_x * prim->verts[1].xyz[0];
      triangle[1].y = emf->scale_y * prim->verts[1].xyz[1];
      triangle[2].x = emf->scale_x * prim->verts[2].xyz[0];
      triangle[2].y = emf->scale_y * prim->verts[2].xyz[1];
...
void gl2psPrintEMFFooter(GLvoid){
  HDC context = ((GL2PSEMF*)gl2ps.stream)->context;
  HENHMETAFILE metafile = CloseEnhMetaFileWithFILE( context );
  (void)DeleteEnhMetaFile( metafile );
...
GLint gl2psEndPage(GLvoid){
  ...
  if(res == GL2PS_SUCCESS){
...
#ifdef GL2PS_USE_EMF
    case GL2PS_EMF:
      phead = gl2psPrintEMFHeader;
      pprim = gl2psPrintEMFPrimitive;
      pfoot = gl2psPrintEMFFooter;
      break;
#endif
#ifdef GL2PS_USE_QT
    case GL2PS_QT:
      phead = gl2psPrintQtHeader;
      pprim = gl2psPrintQtPrimitive;
      pfoot = gl2psPrintQtFooter;
      break;
#endif
    case GL2PS_EPS :
    default :
    ...
    fflush(gl2ps.stream);
#else
#if defined( GL2PS_USE_EMF ) && defined( GL2PS_USE_QT )
    if ( gl2ps.format != GL2PS_EMF && gl2ps.format != GL2PS_QT )
#elif defined( GL2PS_USE_EMF )
    if ( gl2ps.format != GL2PS_EMF )
#else
...
GLvoid gl2psLineStipple(GLuint mode){
  glPassThrough(GL2PS_SET_LINE_STIPPLE);
  glPassThrough(mode);
}
