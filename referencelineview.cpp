                                              ...
                                              const SelectedNames& /*selected*/)
{
    ...
    QAction* cancel_action = page_view->lCMW()->getUi()->cancelReferenceLineAction;
                                              ...
                                              const SelectedNames& selected )
{
    ...
    if ( reference != 0 && !reference->dependsOn( reference_line_->line() ) ) {
        ...
        old_xml.setAttribute( lC::STR::URL,
                              reference_line_->line()->dbURL().toString() );
...
{
    setObjectName( reference_line_->name().toLatin1() );
...
#else
    list_view_item_->setData( lC::formatName( reference_line_->name() )
                              + QString( " <%1>" ).arg( reference_line_->id() ),
                              lC::NAME);
#endif
    list_view_item_->setData( trC( lC::STR::REFERENCE_LINE ), lC::TYPE );
    ...
    line_list_view_->setData(  lC::formatName( reference_line_->name() ) , lC::NAME );
...
{
    if(reference_line_objects_.find(selection_name) != reference_line_objects_.end()) {
        view()->setCursor( QCursor(reference_line_objects_[ selection_name ].cursorShape() ) );
    }
}
                                        ...
                                        const std::vector<GLuint>& items )
{
    if (entity == FIGURE ) {
        ...
        if ( items.size() > 1 && dimensionview_objects_.find( items[1] ) != dimensionview_objects_.end() )
...
{
    QHashIterator< int,GraphicsView > igv( reference_line_objects_ );

    while ( igv.hasNext() )
        if ( (GLuint)igv.key() == selection_name ) return true;
...
void ReferenceLineView::modifiedPosition ( void )
{
    ...
    line_list_view_->setData( reference_line_->line()->detail(), lC::DETAIL );
}
        ...
        if ( dimension_view_ != 0 ) {
            ...
            if ( xml_doc != 0 ) {
                ...
                xml_rep.setAttribute( lC::STR::URL,
                                      reference_line_->dbURL().toString() );
...
void ReferenceLineView::destroyedDmVRef ( void )
{
    ...
    if ( xml_doc != 0 ) {
        QDomElement xml_rep = xml_doc->createElement( lC::STR::DIMENSION_VIEW );
        xml_rep.setAttribute( lC::STR::URL, reference_line_->dbURL().toString() );
        dimension_view_->write( xml_rep );
        ConstraintHistory::instance().appendUnconstraint( xml_rep );
    }
...
{
    reference_line_info_dialog_->getUi()->nameEdit->
            setText( lC::formatName( reference_line_->name() ) );

    if ( reference_line_->line()->isHorizontal() ) {
        reference_line_info_dialog_->getUi()->horizontalButton->setChecked( true );
        reference_line_info_dialog_->getUi()->verticalButton->setChecked( false );
    }
    else {
        reference_line_info_dialog_->getUi()->horizontalButton->setChecked( false );
        reference_line_info_dialog_->getUi()->verticalButton->setChecked( true );
    }

    reference_line_info_dialog_->getUi()->referenceLineEdit->
            setText( lC::formatName( reference_line_->line()->reference()->path() ) );

    reference_line_info_dialog_->getUi()->specifiedRadioButton->setChecked( true );
    reference_line_info_dialog_->getUi()->offsetLengthSpinBox->setEnabled( true );

    // Can't do this yet...
    reference_line_info_dialog_->getUi()->importedRadioButton->setEnabled( false );
    reference_line_info_dialog_->getUi()->constraintChooser->setEnabled( false );

    reference_line_info_dialog_->getUi()->offsetLengthSpinBox->
...
redo:
    ...
    if ( reference_line_info_dialog_->getUi()->nameEdit->isModified() ) {
        // Pageview handles checking the name and putting up the error dialog
        // if necessary.
        int ret = parent()->uniqueFigureName( this,
                                              reference_line_info_dialog_->getUi()->nameEdit->text(),
        ...
        setName( reference_line_info_dialog_->getUi()->nameEdit->text() );
        modified = true;
    }

    if ( reference_line_info_dialog_->getUi()->offsetLengthSpinBox->edited() ) {
        double old_offset = reference_line_->line()->offset();

        if ( reference_line_->line()->offset() >= 0 )
            reference_line_->line()->
                    setOffset( reference_line_info_dialog_->getUi()->offsetLengthSpinBox->length() );
        else
            reference_line_->line()->
                    setOffset( -reference_line_info_dialog_->getUi()->offsetLengthSpinBox->length() );
...
void ReferenceLineView::updateName ( const QString& /*name*/ )
{
    list_view_item_->setData( lC::formatName( reference_line_->name() )
                              + QString( " <%1>" ).arg( reference_line_->id() ),
                              lC::NAME );
    line_list_view_->setData( lC::formatName( reference_line_->line()->name() ),
                              lC::NAME );
    line_list_view_->setData( reference_line_->line()->detail(),  lC::DETAIL );
}
