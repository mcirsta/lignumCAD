...
void AnnotationView::draw ( void ) const
{
    ...
    if ( !annotation_->text().isEmpty() ) {
        ...
        QTextDocument  text( annotation_->text() );
        text.setDefaultFont( font );
        QPalette  cg( parent()->parent()->appPalette() );

        if ( isHighlighted() || isActivated() )
            cg.setColor( QPalette::Text, view()->annotationColor().light() );
        else
            cg.setColor( QPalette::Text, view()->annotationColor() );
        ...
        // formatter...
        view()->qglColor( cg.color( QPalette::Text ) );
        view()->renderText(0, 0, 0, text.toPlainText(), text.defaultFont());
        //TODO ? does this work ?
        ...
        //TODO is the pageSize check correct ?
        if ( (text.pageSize().height()-1) > clip.height() ) {
            glBegin( GL_LINES );
...
{
    if ( annotation_objects_.find( selection_name ) != annotation_objects_.end() )
        view()->setCursor( QCursor( annotation_objects_[ selection_name ].cursorShape() ) );
}
